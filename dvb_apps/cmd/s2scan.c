...
static int TS_OFFSET = 0, TS_IDX = 0, CH_IDX = 0, CH_IDX_END = 0;
static FILE *INFILE;
...
extern void aribstr_to_utf8(unsigned char *src, size_t srclen, unsigned char *dest, size_t destlen);

static void make_output(unsigned char *sdt, int len)
{
	unsigned char *p;
	uint16_t svc_id;
	uint dlen;
...
//fprintf(stderr, "parsing SDT(0x%02hhx) tsid:0x%04hx..\n", sdt[0], sdt[3] << 8 | sdt[4]);
	p = sdt + 11;
	while (p < sdt + len - 4) {
		svc_id = p[0] << 8 | p[1];
		p += 3;
...
int main(int argc, char **argv)
{
	...
	int fe, fd, opt, i, ret, len, adapter = 0, frontend = 0, demux = -1, wtime = 5;
	struct pollfd pfd;
	...
	snprintf(fname, sizeof(fname), "/dev/dvb/adapter%i/frontend%i", adapter, frontend);
	if (demux == -1)
		demux = frontend;
	snprintf(dname, sizeof(dname), "/dev/dvb/adapter%i/demux%i", adapter, demux);
